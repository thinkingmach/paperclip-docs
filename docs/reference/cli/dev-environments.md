...
seo_description: Run a fully isolated ThinkingMach instance per git worktree, or build a deterministic local fixture to develop and test against.
---

# Worktree & Environment Lab

Use these commands when you want a fully isolated ThinkingMach instance per git worktree, or a deterministic local fixture to test against. The `worktree` family creates a worktree-local config, its own embedded Postgres on its own ports, and a seeded copy of another instance's data — so you can run a second ThinkingMach alongside your main one without the two ever touching the same database, ports, or company state. The `env-lab` family spins up disposable infrastructure fixtures (today: an SSH server) so adapter and host-config code has something real to talk to. These are contributor and test-instance tools, not day-to-day company operations.

> **Note:** A "worktree-local instance" is a separate ThinkingMach data directory and database keyed to one git worktree. Its config lives at `<worktree>/.paperclip/config.json` and its data lives under a worktree home (default `~/.paperclip-worktrees`, override with `--home` or `THINKINGMACH_WORKTREES_DIR`). The instance id is derived from the worktree name unless you pass `--instance`.
...
|---|---|
| `thinkingmach worktree:make <name>` | Create `~/paperclip-<name>` as a git worktree, then initialize an isolated instance inside it. |
| `thinkingmach worktree init` | Create repo-local config/env and an isolated instance for the worktree you are already in. |
| `thinkingmach worktree env` | Print shell exports for the current worktree-local instance. |
| `thinkingmach worktree:list` | List git worktrees and flag which look like ThinkingMach worktrees. |
| `thinkingmach worktree:merge-history [source]` | Preview or import issue/comment history from another worktree into the current instance. |
| `thinkingmach worktree reseed` | Re-seed an existing worktree-local instance from another instance or worktree. |
| `thinkingmach worktree repair` | Create or repair a linked worktree-local instance without touching the primary checkout. |
| `thinkingmach worktree:cleanup <name>` | Safely remove a worktree, its branch, and its isolated instance data. |
| `thinkingmach env-lab up` / `status` / `down` / `doctor` | Manage the local SSH env-lab fixture. |

---

## `worktree:make`

The fast path. Creates `~/paperclip-<name>` as a new git worktree (auto-prefixing `paperclip-` if you omit it), then initializes an isolated ThinkingMach instance inside it and seeds its database from a source instance.

```sh
thinkingmach worktree:make feature-x
# creates the worktree at ~/paperclip-feature-x
```

| Flag | Use |
|---|---|
| `--start-point <ref>` | Remote ref to base the new branch on (env: `THINKINGMACH_WORKTREE_START_POINT`). |
| `--instance <id>` | Explicit isolated instance id instead of deriving it from the name. |
| `--home <path>` | Home root for worktree instances (env: `THINKINGMACH_WORKTREES_DIR`, default `~/.paperclip-worktrees`). |
...
Use this when you have **already** created or checked out a git worktree by hand and just want to give it an isolated ThinkingMach instance. It writes the repo-local `.paperclip/config.json` and `.env`, allocates free server and DB ports, and seeds the database — without creating the worktree itself.

```sh
thinkingmach worktree init --name feature-x
```
...
Prints the shell exports that point your terminal at the current worktree-local instance. Source this and every subsequent `thinkingmach` command in the shell targets the worktree's config, home, and instance — not your default `~/.paperclip`.

```sh
eval "$(thinkingmach worktree env)"
```

It emits `THINKINGMACH_CONFIG` (always), plus `THINKINGMACH_HOME`, `THINKINGMACH_INSTANCE_ID`, and `THINKINGMACH_CONTEXT` when present in the worktree's `.env`, followed by any other entries from that `.env`.
...
Lists every git worktree visible from the repo and marks which ones look like ThinkingMach worktrees (i.e. carry a `.paperclip/config.json`). Use it to see what you have spun up before reseeding or cleaning up.

```sh
thinkingmach worktree:list
thinkingmach worktree:list --json
```
...
thinkingmach worktree:merge-history --from paperclip-feature-x --company <company-id>

# Apply the import
thinkingmach worktree:merge-history --from paperclip-feature-x --company <company-id> --apply --yes
```
...
> **Note:** This merges ThinkingMach *data* (issues and comments) between instances. It does not touch git history. To bring source code branches together, use git directly.
...
```sh
thinkingmach worktree reseed --from current --to paperclip-feature-x --yes
```
...
thinkingmach worktree repair

# Bootstrap a worktree for a branch and link an instance to it
thinkingmach worktree repair --branch feature-x
```
...
```sh
thinkingmach worktree:cleanup feature-x
```
...
Deterministic local environment fixtures. Today this is a single SSH server fixture that adapter and host-config code can connect to during development and tests. Every subcommand accepts `-i, --instance <id>` to target a specific ThinkingMach instance (defaults to the current/default instance) and `--json` for machine-readable output. The fixture state is tracked in a `state.json` under the instance's `env-lab/ssh-fixture/` directory.

```sh
thinkingmach env-lab up        # start the SSH fixture
thinkingmach env-lab status    # is it running, and on what host/port?
thinkingmach env-lab doctor    # prerequisites + current status
thinkingmach env-lab down      # stop it
```
...
| Flag | Use |
|---|---|
| `-i, --instance <id>` | ThinkingMach instance id (default: current/default). |
...
```sh
thinkingmach worktree:make feature-x --seed-mode minimal
