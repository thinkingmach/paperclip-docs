...
> **Note:** If a valid config already exists, `onboard` preserves it unchanged, ensures the agent JWT secret, the tool-action signing secret, and the secrets key exist, and prints next-step commands. Use [`configure`](#thinkingmach-configure) to change settings on an existing install rather than re-onboarding.
